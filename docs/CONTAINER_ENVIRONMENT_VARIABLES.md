...
| `AZURE_SUBSCRIPTION_ID` | Azure subscription ID used for best-effort ARM provisioning during startup. | `${AZURE_SUBSCRIPTION_ID}` |

| `CURRENCY_API_BASE_URL` | Base URL for the external currency provider used for rate lookup. If empty/whitespace, defaults to Frankfurter. | `${CURRENCY_API_BASE_URL}` |
